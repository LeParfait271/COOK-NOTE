...
const SITE_UPDATED_AT = '13/08/26';
const APP_RAW_DOWNLOAD_BASE = 'https://raw.githubusercontent.com/LeParfait271/COOK-NOTE/main/downloads';
const ANDROID_LEGACY_APK_VERSION = '5.00';
...
const EXTENDED_TECHNIQUE_GUIDES = Array.isArray(window.COOK_NOTE_EXTRA_TECHNIQUE_GUIDES) ? window.COOK_NOTE_EXTRA_TECHNIQUE_GUIDES : [];
const TECHNIQUE_GUIDES = [...BASE_TECHNIQUE_GUIDES, ...EXTENDED_TECHNIQUE_GUIDES];
...
function shoppingListText(recipes, factorById = {}, ownedItems = {}, mode = 'detailed', pantryItems = []) {
  ...
  return [
    ...
    ...filtered.ownedGroupedItems.map(item => `- ${[formatShoppingAmount(item), item.name].filter(Boolean).join(' ')}${item.ownedByPantry ? ' (déjà à la maison)' : ''}`),
    '',
    '## Détail par recette',
    ...data.blocks
...
function buildMenuShoppingHints(shoppingData) {
  ...
  return [
    ...
    pantry.length && { label: 'Épicerie à vérifier', items: pantry },
...
const RECIPE_SEARCH_SCORE_FIELDS_CACHE = new WeakMap();

function getRecipeSearchScoreFields(recipe, recipesById = {}) {
  const cached = RECIPE_SEARCH_SCORE_FIELDS_CACHE.get(recipe);
  ...
  const value = { title, fields };
  RECIPE_SEARCH_SCORE_FIELDS_CACHE.set(recipe, value);
  return value;
}
...
function scoreRecipeSearch(recipe, query, recipesById = {}) {
  ...
  const { title, fields } = getRecipeSearchScoreFields(recipe, recipesById);
  let score = 0;
...
function getRecipePracticalSections(recipe) {
  ...
  add('pairings', 'Carte des accords', [
...
const RECIPE_INGREDIENT_SEARCH_TEXT_CACHE = new WeakMap();

function getRecipeIngredientSearchText(recipe) {
  const cached = RECIPE_INGREDIENT_SEARCH_TEXT_CACHE.get(recipe);
  if (cached) return cached;
  ...
  RECIPE_INGREDIENT_SEARCH_TEXT_CACHE.set(recipe, value);
  return value;
}
...
function scoreIngredientSearch(recipe, query) {
  ...
  tokens.forEach(token => {
    ...
    if (alternatives.some(item => ingredientText.includes(item) || isCloseSearchToken(item, ingredientText))) matched.push(token);
...
function themeRecipeArtImage(recipe) {
  const theme = isDayArtTheme() ? 'light' : 'dark';
...
function LanguageSwitcher() {
  ...
  const nextLocale = supportedLocales.find(localeCode => localeCode !== locale) || 'fr';
  ...
  return h('div', {
    ...
    h('button', {
      ...
      className: 'language-toggle-code active'
    }, locale.toUpperCase()))
  );
}
...
function Hero() {
  ...
  return h('section', { className: dayArt ? 'hero hero-day-art' : 'hero' },
    ...
    h('span', { className: 'hero-atmosphere hero-atmosphere-circuit', 'aria-hidden': true }),
    ...
    h('section', { ref: modalRef, className: 'modal-panel share-modal', role: 'dialog', 'aria-modal': 'true', 'aria-labelledby': 'share-modal-title', tabIndex: -1, onKeyDown: trapModalFocus, onMouseDown: event => event.stopPropagation() },
      ...
      h('div', { className: 'share-card' },
        h('div', { className: 'share-card-media', style: imageStyle, 'aria-hidden': true },
          !imageUrl && h('span', { className: 'card-letter' }, recipe.title.slice(0, 1))
        ),
...
function SearchPanel({ open, onClose, query, resultQuery, setQuery, searchRef, results, resultMeta, ingredientMeta, openRecipe, rememberSearch }) {
  ...
  useEffect(() => {
    ...
    setDraftQuery(query);
    return () => {
      if (queryCommitTimerRef.current) window.clearTimeout(queryCommitTimerRef.current);
      queryCommitTimerRef.current = 0;
    };
  }, [open]);

  const updateDraftQuery = (value, immediate = false) => {
  ...
  if (!open) return null;
  const hasQuery = Boolean(draftQuery.trim());
  const searchPending = draftQuery !== query || query !== resultQuery;
  ...
  const liveSuggestions = hasQuery && !searchPending ? searchSuggestionTerms(draftQuery, results) : [];
  const rememberCurrentSearch = () => {
    if (draftQuery.trim()) rememberSearch?.(draftQuery);
  };
  ...
  return h('div', { className: 'modal-backdrop search-backdrop', onMouseDown: closeSearch },
    h('section', {
      ...
      h('div', { className: 'modal-head' },
        ...
        h('button', { type: 'button', className: 'icon-btn', onClick: closeSearch, 'aria-label': 'Fermer' }, h(Icon, { name: 'close' }))
      ),
        ...
        h('input', {
          ...
          onKeyDown: event => {
            ...
            if (event.key === 'Enter' && searchPending) {
              event.preventDefault();
              updateDraftQuery(draftQuery, true);
            } else if (event.key === 'Enter' && visibleResults[0]) {
      ...
      !hasQuery && h('div', { className: 'search-quick-rail', 'aria-label': 'Recherches rapides' },
        quickSearches.map(item => h('button', {
          ...
          type: 'button',
          onClick: () => {
            updateDraftQuery(item.query, true);
...
function CommandPalette({
  ...
  const cleanScoringTerm = scoringTerm.trim();
  const normalizedTerm = normalizeText(cleanScoringTerm);
  const commandPending = term !== scoringTerm;
  ...
  const commandRecipes = normalizedTerm
    ? recipes
      .map(recipe => {
        ...
        const textMeta = scoreRecipeSearch(recipe, cleanScoringTerm, recipesById);
        const ingredient = scoreIngredientSearch(recipe, cleanScoringTerm);
        return {
          recipe,
          ...
          commandRecipes.length
            ? h('div', { className: 'command-recipe-list' },
              commandRecipes.map(({ recipe, reason }) => h('button', {
                ...
                h('span', { className: 'command-recipe-image', style: imageBackgroundStyle(displayRecipeImage(recipe)) }),
                h('span', { className: 'command-row-copy' },
                  h('strong', null, recipe.title),
    ...
    h('section', { className: 'modal-panel menu-planner-modal', role: 'dialog', 'aria-modal': 'true', 'aria-labelledby': 'menu-planner-title', tabIndex: -1, onKeyDown: trapModalFocus, onMouseDown: event => event.stopPropagation() },
      ...
      h('div', { className: 'menu-planner-grid' },
        menuItems.map((item, index) => h('article', { key: `${item.key}-${item.recipe.id}-${index}`, className: 'menu-planner-card' },
          h('span', { className: 'menu-planner-image', style: imageBackgroundStyle(displayRecipeImage(item.recipe)), 'aria-hidden': true }),
          h('div', null,
    ...
    Array.from(groups.entries()).map(([role, items]) => h('div', { key: role, className: 'linked-recipe-group' },
      ...
      h('div', { className: 'linked-recipe-list', role: 'list' },
        items.map(item => h('button', {
          ...
          h('span', { className: 'linked-recipe-thumb', style: imageBackgroundStyle(displayRecipeImage(item.recipe)), 'aria-hidden': true }),
    ...
    h('div', { className: 'flavor-map-grid' },
      items.map(item => h('article', { key: item.key, className: 'flavor-node' },
        h('div', { className: 'flavor-node-head' },
          ...
          h('small', null, item.profile)
        ),
        item.matches.length > 0 && h('div', { className: 'flavor-pairs' },
          item.matches.slice(0, 4).map(pair => h('span', { key: `${item.key}:${pair.label}`, title: pair.reason }, pair.label))
        ),
        item.suggestions.length > 0 && h('small', { className: 'flavor-suggestion' }, `A tenter: ${item.suggestions.slice(0, 2).join(', ')}`),
        item.boosters.length > 0 && h('small', { className: 'flavor-boosters' }, `Finition: ${item.boosters.slice(0, 2).join(', ')}`)
      ))
    )
  );
}
...
function App() {
  ...
  const searchFilterQuery = useDebouncedValue(query, 80);
  ...
  const searchPanelResults = useMemo(() => {
    ...
    return searchableRecipes
      .filter(recipe => (searchMeta.has(recipe.id) || ingredientMeta.has(recipe.id)) && recipeMatchesSearchConstraints(recipe, needle, recipesById))
      .sort((a, b) => {
        ...
        return score || a.title.localeCompare(b.title, 'fr');
      });
  }, [searchFilterQuery, searchableRecipes, searchMeta, ingredientMeta, recipesById]);

  const baseFilteredRecipes = useMemo(() => {
    const activeSearchQuery = searchOpen ? '' : searchFilterQuery.trim();
  ...
  const mobileThemeToggleLabel = activeTheme === 'light' ? t('nav.switchToNight') : t('nav.switchToDay');
  const mobileThemeLabel = activeTheme === 'light' ? t('nav.night') : t('nav.day');
  ...
  return h('div', {
    ...
    h('nav', { className: 'mobile-bottom-nav', 'aria-label': 'Navigation mobile' },
      h('button', { type: 'button', onClick: goHome, 'aria-label': t('nav.home'), 'aria-current': !activeRecipe && activePage === 'home' && !onlyFavorites ? 'page' : undefined }, h('span', { className: 'mobile-nav-icon' }, h(Icon, { name: 'home' })), h('span', { className: 'sr-only' }, t('nav.home'))),
      h('button', { type: 'button', onClick: openCommandPalette, 'aria-label': t('nav.search'), 'aria-current': commandOpen || searchOpen ? 'page' : undefined }, h('span', { className: 'mobile-nav-icon' }, h(Icon, { name: 'search' })), t('nav.searchShort')),
      h('button', { type: 'button', onClick: openMenuPlanner, 'aria-label': t('nav.menu'), 'aria-current': menuPlannerOpen ? 'page' : undefined }, h('span', { className: 'mobile-nav-icon' }, h(Icon, { name: 'spark' })), t('nav.menuShort')),
      h('button', { type: 'button', onClick: showFavorites, 'aria-label': t('nav.favorites'), 'aria-current': onlyFavorites ? 'page' : undefined }, h('span', { className: 'mobile-nav-icon' }, h(Icon, { name: 'heart' })), t('nav.favoritesShort')),
      h('button', { type: 'button', onClick: openShoppingBasket, 'aria-label': t('nav.shopping'), 'aria-current': shoppingOpen ? 'page' : undefined }, h('span', { className: 'mobile-nav-icon' }, h(Icon, { name: 'basket' })), t('nav.shoppingShort')),
      h('button', {
        type: 'button',
