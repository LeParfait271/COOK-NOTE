...
const indexAssetVersions = [...indexHtml.matchAll(/\b(?:app|cook-recipes|style)\.(?:js|css)\?v=(\d+)/g)].map(match => match[1]);
const swRegistrationVersion = indexHtml.match(/service-worker\.js\?v=(\d+)/)?.[1];
const swAssetVersions = [...serviceWorker.matchAll(/\b(?:app|cook-recipes|style)\.(?:js|css)\?v=(\d+)/g)].map(match => match[1]);
...
['href="style.css', 'src="assets/vendor', 'src="recipes.js', 'src="cook-recipes.js', 'src="app.js', "register('service-worker.js"].forEach(fragment => {
