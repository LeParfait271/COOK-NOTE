...
expect('Mode tablette ancienne absent.', files.app.includes('lowPowerMode') && files.app.includes('display-low-power') && files.app.includes('Mode tablette ancienne') && files.style.includes('.mc-shell.display-low-power'));
expect('Mode tablette ancienne pas assez optimise.', files.app.includes('hero-wordmark') && files.app.includes('lowPowerMode ? 320 : 120') && files.app.includes('if (!lowPowerMode) loadDeferredCatalogChunks()') && files.app.includes('open ? buildMenuSuggestionWithOverrides') && files.app.includes('open ? buildShoppingListData') && files.style.includes('.mc-shell.display-low-power .hero-wordmark'));
...
expect('Manifest images non charge ou non utilise.', files.index.includes('/assets/image-manifest.js?v=175') && files.index.includes('/app-images.js?v=175') && files.appImages.includes('IMAGE_MANIFEST') && files.appImages.includes('window.CookNoteImages') && files.app.includes('imageSizeAttrs(cardImage)') && files.app.includes('imageBackgroundStyle(recipe.image'));
...
expect('Version footer absente.', files.app.includes("const SITE_VERSION = 'v1.75'") && files.app.includes("const SITE_UPDATED_AT = '24/06/26'") && files.app.includes('site-footer-version'));
