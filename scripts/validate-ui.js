...
const files = {
  ...
  appCooking: fs.readFileSync(path.join(ROOT, 'app-cooking.js'), 'utf8'),
...
expect('Lecture de fiche desktop trop fragmentee.', files.style.includes('.ingredients-panel {\n    position: sticky !important;') && files.style.includes('.steps-panel,\n  .notes-panel {\n    position: relative !important;') && files.rules.includes('trois zones de scroll concurrentes') && files.designSystem.includes('Etapes et les notes gardent le defilement principal'));
...
expect('Mode cuisine incomplet.', files.app.includes('cooking-mode-trigger') && files.app.includes('COOKING_MODE_SCRIPT_SRC') && files.appCooking.includes('window.CookNoteCookingMode') && files.appCooking.includes('trapModalFocus') && files.style.includes('body.cooking-mode-open') && files.style.includes('.cooking-mode-shell') && !files.app.includes('focusMode') && !files.style.includes('recipe-focus-mode') && !files.style.includes('focus-toggle') && !files.style.includes('focus-action'));
