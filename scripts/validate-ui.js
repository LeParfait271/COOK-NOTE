...
expect('Cartes recettes premium incompletes.', files.app.includes("h('div', { className: 'tag-line' }") && files.app.includes("h('div', { className: 'card-facts' }") && files.app.includes("h('div', { className: 'card-quicklook' }") && files.app.includes('getRecipeCardBadges(recipe, recipesById)') && files.style.includes('/* Premium redesign 2026 */') && files.style.includes('.hero-brand-card') && files.style.includes('aspect-ratio: 4 / 5'));
...
expect('Manifest images non charge ou non utilise.', files.index.includes('/assets/image-manifest.js?v=227') && files.index.includes('/app-images.js?v=227') && files.appImages.includes('IMAGE_MANIFEST') && files.appImages.includes('window.CookNoteImages') && files.app.includes('imageSizeAttrs(cardImage)') && files.app.includes('imageBackgroundStyle(recipe.image'));
...
expect('Version footer absente.', files.app.includes("const SITE_VERSION = 'v2.27'") && files.app.includes("const SITE_UPDATED_AT = '30/06/26'") && files.app.includes('site-footer-version'));
