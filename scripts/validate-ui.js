...
expect('Ajouts du mois affiche encore des previews par defaut.', files.app.includes('const visibleRecipes = expanded ? orderedRecipes : []') && files.app.includes('expanded && h(RecipeGrid') && files.app.includes("`Voir les ${recipes.length} ajout") && files.rules.includes('ne doit pas afficher de preview par defaut') && !files.app.includes('orderedRecipes.slice(0, 3)'));
...
expect('Version footer absente.', files.app.includes("const SITE_VERSION = 'v1.33'") && files.app.includes("const SITE_UPDATED_AT = '03/06/26'") && files.app.includes('site-footer-version'));
