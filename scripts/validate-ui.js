...
expect('Manifest images non charge ou non utilise.', Boolean(siteAssetVersion) && files.index.includes(`/assets/image-manifest.js?v=${siteAssetVersion}`) && files.index.includes(`/app-images.js?v=${siteAssetVersion}`) && files.appImages.includes('IMAGE_MANIFEST') && files.appImages.includes('window.CookNoteImages') && files.app.includes('imageSizeAttrs(cardImage)') && files.app.includes('imageBackgroundStyle(recipe.image'));
