...
expect('Version APK Android non decouplee du site.', validators.app.includes("const ANDROID_LEGACY_APK_VERSION = '") && validators.app.includes('cook-note-android-legacy-v${ANDROID_LEGACY_APK_VERSION}.apk') && !validators.app.includes('const APP_VERSION_NUMBER = SITE_VERSION') && validators.androidManual.includes('Version APK Android encore couplee a SITE_VERSION') && validators.ui.includes('ANDROID_LEGACY_APK_VERSION') && rules.includes('Un bump du site ne doit pas forcer une mise a jour APK') && validators.androidWorkflow.includes('peut donc afficher une version web plus recente que l APK') && validators.appsWorkflow.includes("pas de `SITE_VERSION`"));
