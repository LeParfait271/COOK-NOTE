...
- L'APK Android Legacy ne doit plus dependre du WebView systeme. Elle doit embarquer GeckoView ARMv7, demarrer un serveur HTTP local `127.0.0.1` qui sert les assets depuis l APK, puis charger Cook Note avec GeckoView. Garder `minSdk 21`, `usesCleartextTraffic`, `extractNativeLibs`, la permission `INTERNET`, une page d erreur native visible et le routage local `/recette/...`. Les assets sont generes par `scripts/build-android-legacy-assets.js` : JS transpile ES5, `core-js-bundle.min.js`, service worker desactive dans l APK, loader sans CSS moderne (`grid`, `inset`, `min()`), CSS Legacy sans `var()`, `color-mix()`, `clamp()` ni CSS Grid, et runtime ES5 affichant une erreur visible au lieu d un ecran noir. Ne pas copier les assets web modernes bruts dans Android 5. Pour rester sous la limite GitHub des fichiers de 100 MiB, l APK Legacy remplace uniquement dans sa sortie Android les grandes images `recipe-images-optimized` par les miniatures `recipe-card-images`, sans toucher au site HD ni aux recettes.
