...
- Mise a jour groupee obligatoire des applications : quand une app est mise a jour, toutes les entrees app Android doivent etre traitees dans le meme lot. Rebuilder ensemble Android Legacy et Android Modern avec `npm run apps:update-all`, puis commit/push le lot complet. Ne jamais publier un seul APK ou une seule entree app en decalage.
- Qualite apps par generation : seules les vieilles apps peuvent sacrifier la qualite visuelle pour la fluidite. `Android 5.0+` reste Native Lite basse charge ; `Android 8.0+` doit garder le rendu premium `modern-app-hd`, avec detection `CookNoteModernApp/HD` cote Android Modern.
