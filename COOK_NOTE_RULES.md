...
- L'app `Android 5.0+` peut reintegrer des fonctions utiles si elles restent natives et legeres : favoris locaux `SharedPreferences`, derniers ouverts, filtres saison/difficulte, panneau recherche/filtres replie, copie ingredients et bouton natif de mise a jour vers `downloads/cook-note-android-legacy.apk`. Ne pas les remplacer par WebView, React ou moteur web.
...
- L'interface Android Legacy doit garder une lecture native claire : accueil compact, liste `ListView` recyclee avec titres ellipses, recherche et filtres caches dans un panneau replie, fiches organisees en sections, infos rapides en pastilles, ingredients en lignes lisibles, etapes numerotees, favoris, derniers ouverts, filtres saison/difficulte et copie ingredients. Ne pas remplacer cette mise en page par un rendu web ou par de longs blocs texte bruts.
