...
- Les 4 installations du footer doivent rester documentees dans `docs/apps-install-workflow.md` : `Android 5`, `Android recent`, `iOS ancien`, `iOS recent`. Les deux Android pointent vers les assets GitHub Releases via `releases/latest/download` : `cook-note-android-legacy.apk` et `cook-note-android-modern.apk`; les deux iOS ouvrent le guide PWA Safari, sans pretendre telecharger un `.ipa`.
...
- Publier les APK Android sur GitHub Releases uniquement sur demande explicite utilisateur avec `npm run android:legacy:publish-release` ou `npm run android:modern:publish-release`. Le script doit publier dans une release commune `apps-vX.YY`, sans ajouter l'APK dans `dist/` ni dans Git.
- Les assets APK generes dans `android-legacy/app/src/main/assets/www/` et `android-modern/app/src/main/assets/www/`, les dossiers Gradle/build et les fichiers `.apk`/`.aab` ne doivent pas etre versionnes. Le validateur `npm run validate:android` doit rester branche au check.
