...
- Qualite apps par generation : seules les vieilles apps peuvent sacrifier la qualite visuelle pour la fluidite. `Android 5` reste Native Lite basse charge et `iOS ancien` reste PWA simple ; `Android recent` et `iOS recent` doivent garder le rendu premium `modern-app-hd`, avec detection `CookNoteModernApp/HD` cote Android Modern.
