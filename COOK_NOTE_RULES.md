...
- Mise a jour groupee obligatoire des applications : quand une app est mise a jour, toutes les entrees app doivent etre traitees dans le meme lot. Rebuilder ensemble Android Legacy et Android Modern avec `npm run apps:update-all`, verifier les deux entrees iOS PWA du footer, puis commit/push le lot complet. Ne jamais publier un seul APK ou une seule entree app en decalage.
- Qualite apps par generation : seules les vieilles apps peuvent sacrifier la qualite visuelle pour la fluidite. `Android 5.0+` reste Native Lite basse charge et `iOS ancien` reste PWA simple ; `Android 8.0+` et `iOS recent` doivent garder le rendu premium `modern-app-hd`, avec detection `CookNoteModernApp/HD` cote Android Modern.
- L'app `Android 5.0+` peut reintegrer des fonctions utiles si elles restent natives et legeres : favoris locaux `SharedPreferences`, derniers ouverts, filtres saison/difficulte, panneau recherche/filtres replie, copie ingredients, liste de courses locale, copie fiche, partage fiche, rattachements parents additionnels et bouton natif de mise a jour vers `downloads/cook-note-android-legacy.apk`. Ne pas les remplacer par WebView, React ou moteur web.
- Les noms Android installes doivent afficher la version minimale : `Cook Note Android 5.0+` pour Legacy et `Cook Note HD Android 8.0+` pour Modern. Les boutons Android du footer doivent afficher `Android 5.0+` et `Android 8.0+` comme label principal.
- Construire les APK Android uniquement sur demande explicite utilisateur avec `npm run apps:update-all`. Les commandes `npm run android:legacy:update-apk` et `npm run android:modern:update-apk` restent des sous-commandes de diagnostic, pas un workflow de publication final. Le fonctionnement complet doit rester documente dans `docs/android-legacy-workflow.md` et `docs/apps-install-workflow.md`.
- Les 4 installations du footer doivent rester documentees dans `docs/apps-install-workflow.md` : `Android 5.0+`, `Android 8.0+`, `iOS ancien`, `iOS recent`. Les deux Android ouvrent un panneau avec un lien primaire `github.com/LeParfait271/COOK-NOTE/raw/main/downloads/`, un lien secours `raw.githubusercontent.com/LeParfait271/COOK-NOTE/main/downloads/` et la page GitHub du fichier. Le bouton principal utilise le nom APK versionne `cook-note-android-legacy-vX.YY.apk` ou `cook-note-android-modern-vX.YY.apk`; les alias stables `cook-note-android-legacy.apk` et `cook-note-android-modern.apk` restent disponibles pour les mises a jour depuis les apps anciennes. Les deux iOS ouvrent le guide PWA Safari, sans pretendre telecharger un `.ipa`.
- Construire l'APK Android Modern uniquement sur demande explicite utilisateur dans le workflow groupe `npm run apps:update-all`. Le dossier `android-modern/` reste separe de `android-legacy/`.
- L'APK Android Legacy doit rester une app native Java Native Lite pour Android 5/tablettes 2 Go : pas de WebView systeme, pas de GeckoView ARMv7, pas de serveur HTTP local `127.0.0.1`, pas de React, pas de service worker et pas de dossier `assets/www`. Garder `minSdk 21`, une page d erreur native visible, `ListView` recyclee, `CookNoteRepository`, `ImageLoader`, un cache memoire limite et des images decodees en `RGB_565`. Les assets sont generes par `scripts/build-android-legacy-assets.js` dans `android-legacy/build/generated/cook-note-lite` : `recipes-lite.json` sans source externe, `images/` pour miniatures de liste `480px` max, `detail-images/` pour images de fiche `960px` max, recompression `jpeg-js`, aucun asset web moderne brut. Le site HD, Android Modern et les recettes ne doivent pas etre reduits pour cette app.
- L'interface Android Legacy doit garder une lecture native claire : accueil parent Android compact avec les fiches parents du site par defaut, liste `ListView` recyclee avec titres ellipses, recherche et filtres caches dans un panneau replie, puce `Toutes fiches` pour parcourir le catalogue complet sans remettre les enfants en vrac sur l'accueil, fiches organisees en sections, infos rapides en pastilles, ingredients en lignes lisibles, etapes numerotees, favoris, derniers ouverts, filtres saison/difficulte, copie ingredients, liste de courses locale, copie fiche et partage fiche. Les fiches parents Android doivent afficher les recettes en cartes parent visibles comme le site, pas dans un menu deroulant. Les fiches a variantes internes doivent garder un selecteur natif de preparation choisie, pas afficher toutes les variantes a plat. Les fiches parents Android doivent lire `master` et `additionalMasters` pour respecter les rattachements parents additionnels du site. Ne pas remplacer cette mise en page par un rendu web ou par de longs blocs texte bruts.
- Publier les APK Android sur GitHub Releases reste optionnel et uniquement sur demande explicite utilisateur avec `npm run apps:publish-all`. Les commandes `npm run android:legacy:publish-release` et `npm run android:modern:publish-release` restent des sous-commandes internes. Le script doit publier dans une release commune `apps-vX.YY`.
- Les copies APK telechargeables depuis le site sont autorisees uniquement dans `downloads/` apres demande explicite de publication app. A chaque mise a jour app, garder ensemble les alias stables `cook-note-android-legacy.apk`, `cook-note-android-modern.apk` et les copies avec nom APK versionne `cook-note-android-legacy-vX.YY.apk`, `cook-note-android-modern-vX.YY.apk`. Ne jamais les copier dans `dist/` : Cloudflare Pages limite chaque fichier public a 25 MiB. Les assets APK generes dans `android-legacy/build/generated/cook-note-lite/` et `android-modern/app/src/main/assets/www/`, les dossiers Gradle/build et les autres fichiers `.apk`/`.aab` ne doivent pas etre versionnes. Le validateur `npm run validate:android` doit rester branche au check.
