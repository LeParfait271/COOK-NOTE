...
- L'app `Android 5.0+` peut reintegrer des fonctions utiles si elles restent natives et legeres : favoris locaux `SharedPreferences`, recherche simple sans filtres, index recherche precompile, copie ingredients, quantites ajustables, liste de courses locale, courses fusionnees, preferences locales discretes, diagnostic hors ligne, cache image adaptatif, scroll fluide, copie fiche, partage fiche, ecran actif persistant, rattachements parents additionnels et bouton natif de mise a jour vers `downloads/cook-note-android-legacy.apk`. Ne pas les remplacer par WebView, React ou moteur web.
