...
- L'APK Android Legacy doit embarquer une copie WebView ancien generee par `scripts/build-android-legacy-assets.js` : JS transpile ES5, `core-js-bundle.min.js`, service worker desactive dans l APK, loader sans CSS moderne (`grid`, `inset`, `min()`). Ne pas copier les assets web modernes bruts dans Android 5. Le HTML initial Legacy doit etre injecte depuis les assets natifs avec `loadDataWithBaseURL`, permission `INTERNET`, page d erreur native visible et rendu logiciel WebView pour eviter les ecrans noirs Android 5.
