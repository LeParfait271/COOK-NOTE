...
- Les 4 installations du footer doivent rester documentees dans `docs/apps-install-workflow.md` : `Android 5`, `Android recent`, `iOS ancien`, `iOS recent`. Les deux Android pointent vers les APK versionnes sur GitHub Raw via `raw.githubusercontent.com/LeParfait271/COOK-NOTE/main/downloads/` : `cook-note-android-legacy.apk` et `cook-note-android-modern.apk`; les deux iOS ouvrent le guide PWA Safari, sans pretendre telecharger un `.ipa`.
- Construire l'APK Android Modern uniquement sur demande explicite utilisateur avec `npm run android:modern:update-apk`. Le dossier `android-modern/` reste separe de `android-legacy/`.
- Publier les APK Android sur GitHub Releases reste optionnel et uniquement sur demande explicite utilisateur avec `npm run android:legacy:publish-release` ou `npm run android:modern:publish-release`. Le script doit publier dans une release commune `apps-vX.YY`.
