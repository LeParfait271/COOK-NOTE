...
- Les 4 installations du footer doivent rester documentees dans `docs/apps-install-workflow.md` : `Android 5`, `Android recent`, `iOS ancien`, `iOS recent`. Les deux Android pointent vers les APK servis par le site dans `/downloads/` : `cook-note-android-legacy.apk` et `cook-note-android-modern.apk`; les deux iOS ouvrent le guide PWA Safari, sans pretendre telecharger un `.ipa`.
